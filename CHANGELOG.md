...
English 路 [绠�浣撲腑鏂嘳(CHANGELOG.zh-CN.md)
...
- **Fixed installer uploads failing under GitHub Immutable Releases**: keep the Release workflow in draft until all platform artifacts and checksums are uploaded, then publish once so assets can still be attached.
...
- **Main window size and position memory is now opt-in and off by default**: window state is saved and restored across restart or tray rebuild only after users enable it under Settings 鈫?General, so existing users are not affected by position restoration by default.
...
- **Codex API Service Responses WebSocket is now an account-pool routing toggle**: it is off by default for new and existing configurations; only OAuth API Service profiles and their multi-instance profiles use WebSocket after users enable it under Account Pool 鈫?Routing Options, while third-party ProviderGateway profiles remain disabled.
...
- **Main window UI zoom shortcuts (#1601)**: on macOS use 鈱?/鈱? to zoom and 鈱? to reset to 100%; on Windows/Linux use Ctrl combinations; steps match Settings 鈫?General 鈫?UI Scale (90%鈥?50%), persist to `ui_scale`, and survive restarts.
...
- **Codex API Service concurrent image distribution and per-account image limits**: in-flight image generation/edit jobs default to one per account, prefer idle accounts and queue locally; image requests can bypass session affinity; Settings allow 1鈥?6 concurrent image jobs per account. Thanks @phatchau036 for #1578.
...
- **Floating card platform dropdown only lists platforms enabled under Platform Layout 鈫?Show in menu bar**, keeping layout order. Thanks @happyplum for #1596.
...
- **Fixed global tag delete in the account tag editor using only a browser `confirm` and being easy to mis-click**: deleting a suggested 鈥渆xisting tag鈥?now opens an in-app confirm dialog (no overlay dismiss) and only then removes the tag from all accounts. See #645.
...
- **Fixed Antigravity list/card layout forgetting after leaving the page**: view mode always persists, independent of the 鈥渞emember filters鈥?switch. See #1200.
...
- **Grok CLI supports full account export and re-import**: export keeps credentials for recovery; add-account tabs match Codex (OAuth 路 Token / JSON 路 API Key 路 Local import) so you can paste official `auth.json` or Cockpit export JSON, or pick a JSON file.
...
- **Fixed legacy 鈥渄isable image generation鈥?settings that left image gen clickable while the gateway hid `gpt-image-2`**: collection-level `Disabled` / `ImagesOnly` now migrate to `Enabled`, matching the 1.3.4 removal of the disable UI.
...
- **Fixed Grok CLI still showing a 鈥渃urrent鈥?badge when 鈥渟ync official login on switch鈥?is off**: with the switch off, account switches no longer track a global current account and the overview no longer shows the current badge; with it on, official login sync and the current badge still work as before.
...
- **Fixed the 鈥渃opy source instance鈥?dropdown failing to open or closing immediately when creating multi-instance profiles**: the control now uses a stable portal-mounted select so parent re-renders no longer tear down the open menu.
...
- **Global 鈥淩educe motion鈥?setting**: tones down page fades, modal transitions, shadows/blur, and smooth scrolling while keeping essential loading feedback.
- **鈥淎dd to API Service鈥?members support backup marks and a routing shortcut**: toggle backup on each member row and switch routing strategy next to Free-account restrict; custom routing still owns priority/weight, and backup applies under every routing strategy.
...
- **Codex Token / JSON import accepts personal access tokens (`at-鈥 / `personal_access_token`)**: paste a single `at-鈥 line, JSON with that field, or `auth.json`; without refresh/id, auth is stored in the official `personal_access_token` shape (no separate add-account tab). Thanks @daodeqing for the idea and scenario reference in #1448.
- **Settings 鈫?General can set a startup page**: choose a fixed page on cold start, or 鈥淩emember last鈥?to restore the previous page (default).
...
- **Theme color packs (Nord / Tokyo Night / Catppuccin / Gruvbox / Everforest)**: Settings 鈫?General can layer a color pack on light/dark. Thanks @letr1n1ty for the idea and scenario reference in #1399.
...
- **鈥淎dd to API Service鈥?member order matches the account overview**: the dialog no longer has its own sort controls鈥攐verview order is the member-list order.
...
- **Fixed 鈥淪how model-specific quota鈥?not restoring GPT-5.3-Codex-Spark rows**: Spark additional rate limits are no longer dropped in the parser, so the quick-settings switch can show or hide them like other `additional:*` windows. Thanks for the idea and scenario reference in #1540.
...
- **Fixed 鈥淎dd to API Service鈥?member-row plan/column misalignment and hard-to-hit backup toggles**: columns stay aligned; clicking the email selects the row, and the full backup control toggles backup without fighting row selection.
...
- **Fixed Codex overview filters that looked like missing accounts**: when group, tag, search, plan, or folder filters hide some accounts, the page shows visible/total counts, lists active filter chips, and offers one-click clear-all; plan 鈥淎ll鈥?is labeled as all plans so it is not confused with the full account list; stale group filter IDs are dropped after groups load.
- **Fixed auto-switch 鈥渁ll accounts鈥?scope keeping a stale selected-ID list**: switching to or loading `all_accounts` clears residual selected account IDs for Codex and Antigravity so config matches runtime (which already monitors every account in that mode).
...
- **Chinese app multi-open terminology is unified**: Chinese UI, docs, announcements, and runtime messages consistently use `搴旂敤澶氬紑`.
...
- **APIKEY.FUN presentation is clearer in dark mode**: the partner relay copy now says 鈥渙fficial Cockpit partner relay鈥? and the APIKEY.FUN page adds dark-theme styling for panels, inputs, buttons, cards, messages, and key rows.
...
- **Fixed third-party models being treated as unavailable by the local gateway**: models already listed in a provider catalog, such as `deepseek-v4-pro`, no longer fail with an incorrect 鈥渘ot available for this API Key鈥?message.
...
### Changed
- **Codex API/account switching now keeps account changes and session-visibility repair separate**: switching between OAuth accounts, API Key accounts, and the local API Service now completes the real account change first, then shows a post-switch 鈥淐odex Sessions Hidden鈥?dialog with an explicit Repair Visibility action, in-dialog repair results, and a 鈥渄on鈥檛 show again鈥?option.
...
- **Wakeup account pickers now support combined search + type/tag/group filtering with visible-scope batch selection**: task editing, manual tests, and account verification can all filter by account type, tags, and groups (including ungrouped), while 鈥渟elect all鈥?now operates on the currently filtered result set.
...
- **Account injection/startup reliability and operator guidance are improved**: CodeBuddy/CodeBuddy CN/WorkBuddy instance injection now includes post-write verification and clearer 鈥渟ign in manually first鈥?guidance for keychain/state-db failure cases.
...
### Fixed
- **Codex account refresh now recovers from backend-invalidated auth tokens**: quota refresh and account-profile checks now detect `401 token_invalidated`, force one refresh-token exchange, persist the rotated tokens, and retry the official ChatGPT usage/account-check endpoints instead of surfacing a false 鈥渟ign in again鈥?failure while the same account can still be used in the official client.
...
### Fixed
- **Wakeup task creation/test now checks runtime readiness first**: Opening 鈥渘ew task鈥?and 鈥渢est task鈥?now stops early and reuses the existing runtime-path guidance when the wakeup runtime is not configured.
- **Settings and recovery dialogs now surface action failures inline**: Quick Settings path/config errors, file-corruption 鈥渙pen folder鈥?failures, and global modal action failures are now shown in the UI instead of only logging to console.
...
### Changed
- **Codex account cards and tables now display account context**: account rows now show 鈥淧ersonal account鈥?or hydrated team/workspace names based on structure, plan type, and workspace metadata.
...
- **Gemini launch modal wording updated for generic use**: Launch dialog title now uses 鈥淟aunch Instance鈥?instead of a multi-instance-specific label.
...
- **Post-update changelog popup on version jump**: Added startup version-jump detection based on locally recorded `last_run_version`. After an upgrade, the app now shows a 鈥淲hat鈥檚 New鈥?dialog for the current version.
...
### Changed
- **Verification default model selection now prioritizes Flash**: In 鈥淩un check now鈥? the default model now selects the first option whose display name contains `flash` (case-insensitive), and falls back to the first available model when no match exists.
...
- **Instance account selector ordering now follows Accounts sorting across all platforms**: Account dropdown ordering in multi-instance views now reuses each platform鈥檚 Accounts sort logic (Antigravity / Codex / GitHub Copilot / Windsurf / Kiro), avoiding cross-page ordering drift.
...
- **Standardized OAuth copy and tab naming across locales**: Updated add-account OAuth labels/description copy to consistently use 鈥淥Auth Authorization鈥?
...
- **Instance window operation semantics tightened**: 鈥淥pen instance window鈥?now reports focus failures directly instead of auto-starting new processes.
...
### Changed
- **Verification copy and action labels switched from 鈥淰erify鈥?to 鈥淒etect鈥?across all locales**: Added/used `wakeup.verification.actions.runCheckNow`, updated run-hint wording, and aligned the verification-page primary CTA/title.
...
### Changed
- **Banned account UI**: Account cards and table rows now show a 馃敀 `forbidden` status badge and a greyed-out card style to visually distinguish banned accounts.
...
- **Account identity matching overhaul**: Replaced the previous email-only account matching with a strict multi-factor identity matcher (`session_id` 鈫?`refresh_token` 鈫?`email + project_id`), plus a legacy single-email fallback for backward compatibility during upsert.
...
### Added
- **Update Modal**: Unified update check into a modal dialog, including the entry in Settings 鈫?About.
...
### Added
- **Close Confirmation**: New close dialog with minimize/quit actions and a 鈥渞emember choice鈥?option.
- **Close Behavior Setting**: Configure the default close action in Settings 鈫?General.
...
- **Update Checker**: Implemented automatic update checking via GitHub Releases API.
  ...
  - Manual "Check for Updates" button added to **Settings 鈫?About** page with real-time status feedback.
...
- **i18n**: Full internationalization support for 17 languages.
  - 馃嚚馃嚦 绠�浣撲腑鏂? 馃嚬馃嚰 绻侀珨涓枃, 馃嚭馃嚫 English
  - 馃嚡馃嚨 鏃ユ湰瑾? 馃嚢馃嚪 頃滉淡鞏? 馃嚮馃嚦 Ti岷縩g Vi峄噒
  - 馃嚛馃嚜 Deutsch, 馃嚝馃嚪 Fran莽ais, 馃嚜馃嚫 Espa帽ol, 馃嚠馃嚬 Italiano, 馃嚨馃嚬 Portugu锚s
  - 馃嚪馃嚭 袪褍褋褋泻懈泄, 馃嚬馃嚪 T眉rk莽e, 馃嚨馃嚤 Polski, 馃嚚馃嚳 膶e拧tina, 馃嚫馃嚘 丕賱毓乇亘賷丞
