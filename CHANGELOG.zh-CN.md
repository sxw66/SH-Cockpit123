...
- **修复 GitHub Immutable Releases 下安装包无法上传的问题**：Release 工作流在全部平台产物与校验文件上传完成前保持 draft，最终再一次性正式发布，避免过早发布后无法追加附件。
- **修复更新日志编码导致 CI 编译失败的问题**：确保 CHANGELOG 文件保持合法 UTF-8，供 Rust include_str! 正常嵌入。
