...
- **启用应用内自动更新**：发布产物现在带有更新签名，并提供 `latest.json` 更新源。从本版本开始，新版本发布后应用会自动弹出更新提示，也可在“设置 > 通用”开启后台静默自动更新。

### 说明

- **v1.0.6 及更早版本的用户需要手动下载安装本版本一次**：旧版本内置的更新验证公钥已更换，无法自动升级到本版本。安装 v1.0.7 后，后续版本即可全部通过应用内更新完成。
...
- **移除中转站（APIKEY.FUN）相关功能**：删除侧边栏与仪表盘的中转站入口、专属中转站页面数据源、顶部中转推广位，以及 Claude 供应商预设中的 APIKEY.FUN 条目；默认供应商改为 Anthropic 官方。
...
- **缺少 Tauri 签名密钥时发布构建不再失败**：发布流程会在检测到缺少 `TAURI_SIGNING_PRIVATE_KEY` 时改为无签名构建，而不是在所有平台直接中断。

### 说明

- 本次构建未启用更新签名，因此不包含应用内自动更新所需的产物。
